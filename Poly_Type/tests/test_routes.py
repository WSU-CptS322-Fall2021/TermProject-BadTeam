...
def test_close_open_challenge(test_client, init_database):
    ...
    tc = db.session.query(Challenge).filter(Challenge.title=='test_challenge').first()
    #Close the challenge
    response = test_client.post('/close_challenge/'+str(tc.id), data=dict(challengeid=tc.id), follow_redirects=True)
    #Check the challenge closed
    assert response.status_code == 200
    assert tc.open == False
    #Open the challenge.
    response = test_client.post('/open_challenge/'+str(tc.id), data=dict(challengeid=tc.id), follow_redirects=True)
    #Check the challenge is open
    assert response.status_code == 200
    assert tc.open == True
